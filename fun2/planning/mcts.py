...
from fun2.planning.mcts_continuous_action_space import MCTS, Node
...
class MCTSRunnerCPU:
    ...
    def run_on_cpu(self, chunk_size=500) -> pd.DataFrame:
        ...
        while bin_start < self.matrix.shape[0]:
            ...
            if not chunk_summits.empty:
                ...
                for row in chunk_summits.itertuples(index=False):
                    ...
                    node, reward, quality, intensity, p_up, p_dn, d_up, d_dn = mcts_solver.search()
                    ...
                    if node is None:
                        ...
                        results.append(
                            ...
                            intensity, quality, p_up, p_dn, d_up, d_dn)
                        )
                    else:
                        ...
                        results.append(
                            ...
                            reward, intensity, quality, p_up, p_dn, d_up, d_dn)
                        )
        ...
        df = pd.DataFrame(results, columns=['chrom', 'summit_start', 'summit_end', 'identified_center', 'height', 'width', 'angle',
         'elongation_up', 'elongation_down', 'width_up', 'width_down', 'reward', 'intensity', 'quality', 'pval_upstream', 'pval_downstream', 'rb_upstream', 'rb_downstream'])

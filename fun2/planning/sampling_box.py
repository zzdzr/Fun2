...
class SamplingBoxAgent:
    ...
    def compute_sum_intensity(self, box: np.ndarray) -> float:
    ...
    def calculate_intensity(self) -> Tuple[float, float, float]:
        ...
        box = self.generate_box()
        # up_box, down_box = self._generate_padding_box()
        
        # downstream_background, _, _, _, _ = self._calculate_intensity_within_box(down_box)
        # upstream_background, _, _, _, _ = self._calculate_intensity_within_box(up_box)
        sampling, _, _, _, _ = self._calculate_intensity_within_box(box)
        # sampling = self._calculate_intensity_within_box(box)
        # upstream_fc = sampling / (upstream_background + 1e-9)
        # downstream_fc = sampling / (downstream_background + 1e-9)
